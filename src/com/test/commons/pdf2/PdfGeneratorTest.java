...
public class PdfGeneratorTest extends TestCase {
	...
	/** 以程式完全控制的方式繪出公文內容 */
	public void testDrawTextFlow() throws Exception {
...
//        pdf.setPlane15Font(plane15Font);
        ...
        xy = pdf.drawTextFlow("地　　址：10422 台北市中山路三段22號", 4, 125, 33, 21);
        xy = pdf.drawTextFlow("聯 絡 人：郭大同", 4, 125, ++xy[1], 21);
        xy = pdf.drawTextFlow("聯絡電話：12345678", 4, 125, ++xy[1], 21);
        xy = pdf.drawTextFlow("傳真電話：87654321", 4, 125, ++xy[1], 21);

        xy = pdf.drawTextFlow("受文者：大同公司", 5, 15, 50, 21);
        xy = pdf.drawTextFlow("發文日期：中華民國93年12月30日", 4, 15, ++xy[1] + 4, 21);
        xy = pdf.drawTextFlow("發文字號：測字第0930001004號", 4, 15, ++xy[1], 21);
        xy = pdf.drawTextFlow("速別：普通件", 4, 15, ++xy[1], 21);
        xy = pdf.drawTextFlow("密等及解密條件或保密期限：密，總有一天", 4, 15, ++xy[1], 21);
        xy = pdf.drawTextFlow("附件：", 4, 15, ++xy[1], 21);

        xy = pdf.drawTextFlow("主旨：我是主旨。", 4.5F, 15, ++xy[1] + 4, 12);
        xy = pdf.drawTextFlow("說明：", 4.5F, 15, ++xy[1], 12);
        xy = pdf.drawTextFlow(new StringBuilder("測外字：U+2F81A='").appendCodePoint(0x2F81A).append("',")
        		...
        		.toString(), 4.5F, 20, ++xy[1], 12, 150, 
        		PdfConst.FONT_MING, PdfConst.FONT_STYLE_BOLD, PdfConst.COLOR_RED);
        xy = pdf.drawTextFlow("一、說明一", 4.5F, 20, ++xy[1], 12);
        xy = pdf.drawTextFlow("(一)說明一(一)", 4.5F, 25, ++xy[1], 12);
        xy = pdf.drawTextFlow("(二)說明一(二)", 4.5F, 25, ++xy[1], 12);
        xy = pdf.drawTextFlow("１、說明一(二)１", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("２、說明一(二)２", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("(１)說明一(二)２(１)", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("(２)說明一(二)２(２)", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("甲、說明一(二)２(２)甲", 4.5F, 35, ++xy[1], 12);
        xy = pdf.drawTextFlow("乙、說明一(二)２(２)乙", 4.5F, 35, ++xy[1], 12);
        xy = pdf.drawTextFlow("(甲)說明一(二)２(２)乙(甲)", 4.5F, 40, ++xy[1], 12);
        xy = pdf.drawTextFlow("(乙)說明一(二)２(２)乙(乙)", 4.5F, 40, ++xy[1], 12);
        xy = pdf.drawTextFlow("(丙)說明一(二)２(２)乙(丙)", 4.5F, 40, ++xy[1], 12);
        xy = pdf.drawTextFlow("丙、說明一(二)２(２)丙", 4.5F, 35, ++xy[1], 12);
        xy = pdf.drawTextFlow("(３)說明一(二)２(３)", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("３、說明一(二)３", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("(三)說明一(三)", 4.5F, 25, ++xy[1], 12);
        xy = pdf.drawTextFlow("二、說明二", 4.5F, 20, ++xy[1], 12);
        xy = pdf.drawTextFlow("三、說明三", 4.5F, 20, ++xy[1], 12);

        xy = pdf.drawTextFlow("辦法：我是辦法", 4.5F, 15, ++xy[1], 12);
        xy = pdf.drawTextFlow("一、辦法一", 4.5F, 20, ++xy[1], 12);
        xy = pdf.drawTextFlow("(一)辦法一(一)", 4.5F, 25, ++xy[1], 12);
        xy = pdf.drawTextFlow("(二)辦法一(二)", 4.5F, 25, ++xy[1], 12);
        xy = pdf.drawTextFlow("１、辦法一(二)１", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("２、辦法一(二)２", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("(１)辦法一(二)２(１)", 4.5F, 35, ++xy[1], 12);
        xy = pdf.drawTextFlow("(２)辦法一(二)２(２)", 4.5F, 35, ++xy[1], 12);
        xy = pdf.drawTextFlow("甲、辦法一(二)２(２)甲", 4.5F, 40, ++xy[1], 12);
        xy = pdf.drawTextFlow("乙、辦法一(二)２(２)乙", 4.5F, 40, ++xy[1], 12);
        xy = pdf.drawTextFlow("(甲)辦法一(二)２(２)乙(甲)", 4.5F, 45, ++xy[1], 12);
        xy = pdf.drawTextFlow("(乙)辦法一(二)２(２)乙(乙)", 4.5F, 45, ++xy[1], 12);
        xy = pdf.drawTextFlow("(丙)辦法一(二)２(２)乙(丙)", 4.5F, 45, ++xy[1], 12);
        xy = pdf.drawTextFlow("(丁)辦法一(二)２(２)乙(丁)", 4.5F, 45, ++xy[1], 12);
        xy = pdf.drawTextFlow("(戊)辦法一(二)２(２)乙(戊)", 4.5F, 45, ++xy[1], 12);
        xy = pdf.drawTextFlow("丙、辦法一(二)２(２)丙", 4.5F, 40, ++xy[1], 12);
        xy = pdf.drawTextFlow("(３)辦法一(二)２(３)", 4.5F, 35, ++xy[1], 12);
        xy = pdf.drawTextFlow("３、辦法一(二)３", 4.5F, 30, ++xy[1], 12);
        xy = pdf.drawTextFlow("(三)辦法一(三)", 4.5F, 25, ++xy[1], 12);
        xy = pdf.drawTextFlow("二、辦法二", 4.5F, 20, ++xy[1], 12);
        xy = pdf.drawTextFlow("三、辦法三 一二三四五六七八九十壹貳參肆伍陸柒捌玖拾一二三四五六七八九十壹貳參肆伍陸柒捌玖拾一二三四五六七八九十壹貳參肆伍陸柒捌玖拾一二三四五六七八九十壹貳參肆伍陸柒捌玖拾一二三四五六七八九十壹貳參肆伍陸柒捌玖拾", 4.5F, 20, ++xy[1], 10);

        xy = pdf.drawTextFlow("正本：大同世界公司，國防部", 4, 15, ++xy[1], 12);
        xy = pdf.drawTextFlow("副本：小異公司", 4, 15, ++xy[1], 12);
    ...
    public void xtestGrid() throws Exception {
...
//        testIText_drawTimeSlots2(writer.getDirectContent());
    ...
    void xtestIText_drawTimeSlots1(PdfContentByte directcontent) {
        directcontent.saveState();
        float x;
...
//directcontent.stroke();
    ...
    void xtestIText_drawTimeSlots2(PdfContentByte directcontent) {
        directcontent.saveState();
        float x;
